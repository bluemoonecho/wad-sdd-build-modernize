...
export function incrementDuckStock(
  lineItems: StockDecrementLineItem[],
  options?: CatalogDataSourceOptions,
): void {
  const aggregatedQuantities = new Map<string, number>();
  ...
  for (const lineItem of lineItems) {
    if (!Number.isInteger(lineItem.quantity) || lineItem.quantity <= 0) {
      throw new Error(`Invalid stock increment quantity for duck "${lineItem.duckId}".`);
    }
  ...
  for (const duckId of aggregatedQuantities.keys()) {
    if (!catalogByDuckId.has(duckId)) {
      throw new DuckNotFoundError(duckId);
    }
  }

  const updatedCatalog = catalog.map((duck) => {
    const restoreQuantity = aggregatedQuantities.get(duck.id);
    ...
    return {
      ...duck,
      stockCount: duck.stockCount + restoreQuantity,
    };
  });
